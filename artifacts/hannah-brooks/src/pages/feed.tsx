...
// Inline video player card for the grid
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-3 left-3 right-3 z-10 flex items-center justify-between">
        <button
          onClick={toggle}
          className="w-9 h-9 rounded-full bg-black/60 backdrop-blur border border-white/20 flex items-center justify-center hover:bg-black/80 transition-colors"
        >
          {playing ? <Pause className="w-4 h-4 text-white fill-white" /> : <Play className="w-4 h-4 text-white fill-white ml-0.5" />}
        </button>
        <div className="flex items-center gap-2">
          <button onClick={toggleMute}
            className="w-7 h-7 rounded-full bg-black/60 backdrop-blur border border-white/10 flex items-center justify-center hover:bg-black/80 transition-colors">
      ...
      {post.caption && (
        <div className="absolute bottom-14 left-3 right-3 z-10">
          <p className="text-white text-xs line-clamp-2 leading-relaxed drop-shadow">{post.caption}</p>
        </div>
      )}
...
// Image card for the grid
function ImageCard({ post, onClick, index }: { post: Post; onClick: () => void; index: number }) {
  return (
    <div
      onClick={onClick}
      className="relative break-inside-avoid rounded-2xl overflow-hidden cursor-pointer group border border-white/5 hover:border-amber-400/30 transition-all duration-300 hover:shadow-[0_8px_30px_rgba(201,168,76,0.15)] hover:-translate-y-0.5 block"
    >
      <img
...
// Full-screen lightbox with video support
function Lightbox({ post, onClose }: { post: Post; onClose: () => void }) {
  ...
  useEffect(() => {
    if (isVideo && vidRef.current) {
      vidRef.current.play().catch(() => setPlaying(false));
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/97 backdrop-blur-xl flex items-center justify-center p-4"
      onClick={onClose}>
      <div className="relative max-w-2xl w-full bg-zinc-900/90 border border-white/10 rounded-3xl overflow-hidden shadow-2xl"
          ...
          {isVideo ? (
            <div className="relative">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                ...
                <button onClick={toggleMute}
                  className="w-9 h-9 rounded-full bg-black/70 backdrop-blur border border-white/20 flex items-center justify-center hover:bg-black/90 transition-colors">
        ...
        <div className="p-5">
          ...
          {post.caption && <p className="text-white/90 text-sm leading-relaxed">{post.caption}</p>}
        </div>
      </div>
    </div>
  );
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {vipPosts.map((post) => {
                      ...
                      return (
                        ...
                        >
                          <img
                            src={resolveUrl(post.thumbnailUrl || post.imageUrl)}
                            alt="VIP content"
