...
const RECENT_ACTIVITY = [
  { time: "2m ago", text: "Persona 'Hannah AI' replied to 3 messages autonomously", color: "var(--neon)" },
  { time: "15m ago", text: "Voice conversion latency: 84ms average over last 100 frames", color: "#00ff88" },
  { time: "1h ago", text: "Face model retrained with 2 new reference frames", color: "var(--neon2)" },
  { time: "2h ago", text: "Auto-reply mode: 12 messages sent, 0 flagged", color: "#ffaa00" },
  { time: "3h ago", text: "OBS virtual camera connected — streaming active", color: "var(--neon)" },
];

const SYSTEM_METRICS = [
  { label: "GPU VRAM", value: 72, color: "var(--neon)", max: 100 },
  { label: "CPU Load", value: 34, color: "#00ff88", max: 100 },
  { label: "Memory", value: 58, color: "var(--neon2)", max: 100 },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Active Personas" value={3} icon={<Users size={16} />} color="var(--neon)" trend="1 this week" trendUp />
        <StatCard label="Avg Latency" value={87} unit="ms" icon={<Zap size={16} />} color="#00ff88" trend="12ms" trendUp={false} />
        <StatCard label="Auto-replies Today" value={142} icon={<MessageSquare size={16} />} color="var(--neon2)" trend="38%" trendUp />
        <StatCard label="Sessions Live" value={1} icon={<Monitor size={16} />} color="#ff006e" />
      </div>

      <div className="grid lg:grid-cols-3 gap-6">
        {/* Quick actions */}
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-sm font-bold mono uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.3)" }}>Quick Actions</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { page: "live" as Page, icon: <Monitor size={20} />, title: "Live Preview", desc: "Start face & voice transformation", color: "#ff006e", badge: "WEBCAM" },
              { page: "training" as Page, icon: <Brain size={20} />, title: "Train Persona", desc: "Upload video & audio samples", color: "var(--neon2)", badge: null },
              { page: "chat" as Page, icon: <MessageSquare size={20} />, title: "Chat Control", desc: "Manage auto-reply engine", color: "var(--neon)", badge: "142 queued" },
          ...
          {/* Recent activity */}
          <h2 className="text-sm font-bold mono uppercase tracking-widest pt-2" style={{ color: "rgba(255,255,255,0.3)" }}>Recent Activity</h2>
        ...
        <div className="space-y-4">
          <h2 className="text-sm font-bold mono uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.3)" }}>System Health</h2>
          <GlassCard className="p-5 space-y-5">
            ...
            <div className="pt-3 border-t space-y-2" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
              {[
                ...
                { label: "Voice Engine", status: "Active", color: "var(--neon)" },
                { label: "LLM (GPT-4o)", status: "Connected", color: "var(--neon)" },
                { label: "OBS Camera", status: "Running", color: "#00ff88" },
                { label: "Auto-Reply", status: "Autonomous", color: "var(--neon2)" },
              ].map((s) => (
