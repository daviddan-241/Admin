...
export default function LivePreview() {
  ...
  const CONTROLS = [
    { label: "Face Swap", desc: "LivePortrait + InsightFace", active: faceSwap, toggle: () => setFaceSwap(!faceSwap), color: "var(--gold)" },
    { label: "Voice Convert", desc: "RVC v2 real-time", active: voiceActive, toggle: () => setVoiceActive(!voiceActive), color: "var(--gold2)" },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl glass border ${cameraActive ? "border-green-500/30" : "border-white/10"}`}>
          ...
          <span className="text-xs mono font-bold" style={{ color: cameraActive ? "#00ff88" : "rgba(232,223,200,0.35)" }}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <GlassCard className="overflow-hidden" style={{ border: cameraActive && faceSwap ? "1px solid var(--gold)" : "1px solid rgba(255,255,255,0.06)" } as React.CSSProperties}>
            <div className="relative aspect-video bg-black/80 flex items-center justify-center">
              ...
              {!cameraActive && (
                <div className="flex flex-col items-center gap-4 text-center px-6">
                  <div className="w-20 h-20 rounded-full flex items-center justify-center"
                    style={{ background: "rgba(201,168,76,0.05)", border: "2px solid rgba(201,168,76,0.2)" }}>
              ...
              {cameraActive && (
                <>
                  {/* Scan corners */}
                  <div className="absolute top-3 left-3 w-6 h-6 border-l-2 border-t-2 pointer-events-none" style={{ borderColor: "var(--gold)" }} />
                  <div className="absolute top-3 right-3 w-6 h-6 border-r-2 border-t-2 pointer-events-none" style={{ borderColor: "var(--gold)" }} />
                  <div className="absolute bottom-3 left-3 w-6 h-6 border-l-2 border-b-2 pointer-events-none" style={{ borderColor: "var(--gold)" }} />
                  <div className="absolute bottom-3 right-3 w-6 h-6 border-r-2 border-b-2 pointer-events-none" style={{ borderColor: "var(--gold)" }} />
          ...
          {cameraActive && faceSwap && (
            <GlassCard className="p-5">
              <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
                <Sliders size={16} style={{ color: "var(--gold)" }} /> Fine Controls
              </h3>
              <div className="space-y-4">
                {[
                  { label: "Blend Strength", value: 85, color: "var(--gold)" },
                  { label: "Expression Transfer", value: 70, color: "var(--gold2)" },
          ...
          <GlassCard className="p-5" style={{ border: "1px solid rgba(201,168,76,0.15)" } as React.CSSProperties}>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-black"
                style={{ background: "linear-gradient(135deg,var(--gold),var(--gold2))" }}>H</div>
              <div>
            ...
            <GlassCard className="p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: "Voice Latency", value: voiceActive ? "91ms" : "—", color: "var(--gold2)" },
                  { label: "Render Quality", value: "1080p", color: "var(--gold)" },
                  { label: "OBS Status", value: obsConnected ? "Connected" : "Disconnected", color: obsConnected ? "#00ff88" : "rgba(232,223,200,0.35)" },
                ].map((m) => (
